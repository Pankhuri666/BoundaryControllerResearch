...
#define USE_PAPER_BOUNDARY_CONTROLLER 1
#include "LaneFree.h"
FILE *logFile = NULL;
...
logFile = fopen("paper_controller_log.csv", "w");

if (logFile != NULL)
{
    fprintf(logFile,
            "time,y,left,right,yd,ey,theta,ux,uy\n");
    fflush(logFile);
}
}
		...
		width = get_edge_width(myedges[i]);
	 	if(n_edge_ids>0){
			...
			for (j = 0; j < n_edge_ids; j++) {
				...
				#if USE_PAPER_BOUNDARY_CONTROLLER

				// =====================================================
// Paper Controller - Stage 1
// Boundary Controller
// Based on:
// "Controlling Automated Vehicles on Large Lane-Free
// Roundabouts"
// =====================================================
...
double y = pos_y;
double vy = speed_y;
double theta = get_veh_orientation(myID);
...
// Boundary distances
get_distance_to_road_boundaries_at(
    myID,
    lookAhead,
    0,
    &left_boundary_dist,
...
// Next:
// 1. Compute desired lateral position
// 2. Compute boundary error
// 3. Compute steering command
// Desired lateral position
double yd = y;

// If too close to the left boundary
if (left_boundary_dist < width / 2.0 + safetyMargin)
{
    yd = y - ((width / 2.0 + safetyMargin) - left_boundary_dist);
}

// If too close to the right boundary
else if (right_boundary_dist < width / 2.0 + safetyMargin)
{
    yd = y + ((width / 2.0 + safetyMargin) - right_boundary_dist);
}
// Lateral tracking error
double ey = y - yd;
// State-feedback gains (Eq. 29)
double Ky = 0.30;
double Ktheta = 1.20;
// Steering command
// State-feedback lateral controller (Paper Eq. 29)
double uy = -(Ky * ey + Ktheta * theta);
// Longitudinal control (maintain desired speed)
double ux = erfc(speed - des_speed) - 1;
      printf(
"[PAPER CTRL] "
"y=%.3f "
"L=%.3f "
...
if (logFile != NULL)
{
    fprintf(logFile,
            ...
            get_current_time_step() * get_time_step_length(),
            y,
            left_boundary_dist,
            right_boundary_dist,
            yd,
...
apply_acceleration(myID, ux, uy);
				...
				if (left_boundary_dist < width / 2.) {
					...
					yd = pos_y - ((width / 2.) - left_boundary_dist) - y_const;
				}
				else if (right_boundary_dist < width / 2.) {
					yd = pos_y + ((width / 2.) - right_boundary_dist) + y_const;
				}
				double K_p = 1, K_d = 2;

				// simple PD boundary controller
				uy = K_d * (-speed_y) + K_p * (yd - pos_y);
